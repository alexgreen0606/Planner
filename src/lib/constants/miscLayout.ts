// ✅

export const MODAL_INPUT_HEIGHT = 50;

export const TOOLBAR_HEIGHT = 42;

export const THIN_LINE_HEIGHT = 15;

export const HEADER_HEIGHT = 46;

export const BOTTOM_NAVIGATION_HEIGHT = 100;

export const PAGE_LABEL_HEIGHT = 40;

export const PLANNER_CARD_HEADER_HEIGHT = 60;

export const PLANNER_CAROUSEL_ICON_WIDTH = 42;

export const PLANNER_BANNER_PADDING = 6;

export const PLANNER_CHIP_HEIGHT = 24;

export const PLANNER_CAROUSEL_DAY_OF_WEEK_FONT_SIZE = 9;

// Tailwind 4
export const LARGE_MARGIN = 16;

// Tailwind 2
export const SMALL_MARGIN = 8;

export const PLANNER_CAROUSEL_HEIGHT =
  PLANNER_CAROUSEL_ICON_WIDTH + PLANNER_CAROUSEL_DAY_OF_WEEK_FONT_SIZE;
