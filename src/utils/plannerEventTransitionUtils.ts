...
export async function transitionToAllDayCalendarEvent(
  initialState: TInitialEventMetadata,
  newRange: TDateRange,
  newCalendarId: string
) {
  ...
  switch (initialState.eventType) {
    ...
    case EEventType.CALENDAR_ALL_DAY: {
      ...
      calendarEventId = await getCarryoverCalendarEventIdByCalendarEvent(
        calendarEvent,
        newCalendarId
      );
      wasAllDayEvent = true;
    ...
    case EEventType.CALENDAR_MULTI_DAY: {
      // CALENDAR_MULTI_DAY → CALENDAR_ALL_DAY
      const { startPlannerEvent, endPlannerEvent, startPlanner, endPlanner, calendarEvent } =
        initialState;
    ...
    case EEventType.CALENDAR_SINGLE_DAY: {
      // CALENDAR_SINGLE_DAY → CALENDAR_ALL_DAY
      const { plannerEvent, planner } = initialState;
      const { timeConfig } = plannerEvent;

      // Grab the event's ID from the calendar if it is not moving to a new calendar.
      calendarEventId = await getCarryoverCalendarEventIdByPlannerEvent(
        plannerEvent,
        newCalendarId
      );
...
export async function transitionToSingleDayCalendarEvent(
  initialState: TInitialEventMetadata,
  newRange: TDateRange,
  newCalendarId: string
) {
  ...
  switch (initialState.eventType) {
    case EEventType.NON_CALENDAR: {
      // NON_CALENDAR → CALENDAR_SINGLE_DAY
      const { plannerEvent, planner } = initialState;
    ...
    case EEventType.CALENDAR_ALL_DAY: {
      // CALENDAR_ALL_DAY → CALENDAR_SINGLE_DAY
      const { calendarEvent } = initialState;
    ...
    case EEventType.CALENDAR_MULTI_DAY: {
      // CALENDAR_MULTI_DAY → CALENDAR_SINGLE_DAY
      const { startPlannerEvent, endPlannerEvent, startPlanner, endPlanner, calendarEvent } =
        initialState;
...
export async function transitionToMultiDayCalendarEvent(
  initialState: TInitialEventMetadata,
  newRange: TDateRange,
  newCalendarId: string
) {
  ...
  switch (initialState.eventType) {
    case EEventType.NON_CALENDAR: {
      // NON_CALENDAR → CALENDAR_MULTI_DAY
      const { plannerEvent, planner } = initialState;
      ...
      // Re-use the position and ID of the existing event.
      carryoverEventMetadata[ECarryoverEventType.START_EVENT] = getCarryoverEventMetadata(
        startIso,
        plannerEvent,
        planner
      );
    ...
    case EEventType.CALENDAR_ALL_DAY: {
      // CALENDAR_ALL_DAY → CALENDAR_MULTI_DAY
      const { calendarEvent } = initialState;
    ...
    case EEventType.CALENDAR_MULTI_DAY: {
      ...
      carryoverEventMetadata[ECarryoverEventType.END_EVENT] = getCarryoverEventMetadata(
        endIso,
        endPlannerEvent,
        endPlanner
      );
    ...
    case EEventType.CALENDAR_SINGLE_DAY: {
      // CALENDAR_SINGLE_DAY → CALENDAR_MULTI_DAY
      const { plannerEvent, planner } = initialState;
      const { timeConfig } = plannerEvent;
...
export async function transitionToNonCalendarEvent(
  initialState: TInitialEventMetadata,
  newRange: TDateRange
) {
  const { startIso } = newRange;
  ...
  switch (initialState.eventType) {
    ...
    case EEventType.CALENDAR_MULTI_DAY: {
      // CALENDAR_MULTI_DAY → NON_CALENDAR
      const { startPlannerEvent, endPlannerEvent, startPlanner, endPlanner, calendarEvent } =
        initialState;
    ...
    case EEventType.CALENDAR_SINGLE_DAY: {
      // CALENDAR_SINGLE_DAY → NON_CALENDAR
      const { plannerEvent, planner } = initialState;
